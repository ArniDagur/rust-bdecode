...
fn test_torrent_file_bytes(bytes: &[u8]) {
    ...
    // Check announce list-of-lists without using iterators
    ...
    // Do the same thing except use iterator syntax
    for any in announce_list.iter() {
        let alternatives = any.as_list().unwrap();
